...
-- Script date 13/10/2013 09:11:19 p.m.
...
--
-- Definition for table tbl_oficina
--
...
CREATE TABLE IF NOT EXISTS tbl_componente (
  ...
  INDEX Reftbl_oficina1 (id_oficina),
  CONSTRAINT Reftbl_oficina1 FOREIGN KEY (id_oficina)
    REFERENCES tbl_oficina(id_oficina)
)
ENGINE = INNODB
AUTO_INCREMENT = 99
AVG_ROW_LENGTH = 1489
...
--
-- Definition for procedure SPRABMOficina
--
DROP PROCEDURE IF EXISTS SPRABMOficina$$
CREATE PROCEDURE SPRABMOficina(IN `pIdAbm` TINYINT,IN pId_oficina INT, IN `pOficina` VARCHAR(100), IN `pDescripcion_oficina` VARCHAR(100), IN `pActivo` CHAR(1))
BEGIN
  IF pIdAbm = 1
        THEN
...
--
-- Definition for procedure SPRCNSAcompaniante
--
DROP PROCEDURE IF EXISTS SPRCNSAcompaniante$$
CREATE PROCEDURE SPRCNSAcompaniante(IN pId_sujeto INT(11))
BEGIN
  SELECT
          viw.id_sujeto,
...
--
-- Definition for procedure SPRCNSAlumnoxUsuario
--
DROP PROCEDURE IF EXISTS SPRCNSAlumnoxUsuario$$
CREATE PROCEDURE SPRCNSAlumnoxUsuario(IN pId_Usuario INT(11))
BEGIN
  SELECT
...
--
DROP PROCEDURE IF EXISTS SPRCNSFiltro$$
CREATE PROCEDURE SPRCNSFiltro(IN pIdFiltro INT, IN pCns VARCHAR(50), IN pLimit INT, IN pOffset INT, OUT pRow INT)
BEGIN

  SET @vLimit = pLimit;
  SET @vEmpiezo = pOffset;
  ...
  IF pIdFiltro = 0 THEN
    ...
    INTO
      pRow
    FROM
      sis_biblio.viw_tesis ts
    WHERE
      ts.titulo LIKE CONCAT('%', pCns, '%');
    IF pRow > 10 THEN
      PREPARE STMT FROM "	SELECT * FROM viw_tesis ts WHERE ts.titulo LIKE CONCAT('%',pCns,'%') LIMIT ?,?";
      EXECUTE STMT USING @vLimit, @vEmpiezo;
  ...
  ELSE
    IF pIdFiltro = 1 THEN
      ...
      FROM
        viw_tesis ts
      WHERE
        ts.alumno LIKE CONCAT('%', pCns, '%');
      IF pRow > 10 THEN
        PREPARE STMT FROM "	SELECT * FROM viw_tesis ts WHERE ts.alumno LIKE CONCAT('%',pCns,'%') LIMIT ?,?";
        EXECUTE STMT USING @vLimit, @vEmpiezo;
      ELSE
        SELECT
          *
    ...
    ELSE
      IF pIdFiltro = 2 THEN
        ...
        FROM
          viw_tesis ts
        WHERE
          ts.docente LIKE CONCAT('%', pCns, '%');
        IF pRow > 10 THEN
          PREPARE STMT FROM "	SELECT * FROM viw_tesis ts WHERE ts.docente LIKE CONCAT('%',pCns,'%') LIMIT ?,?";
          EXECUTE STMT USING @vLimit, @vEmpiezo;
        ELSE
          SELECT
            *
      ...
      ELSE
        IF pIdFiltro = 3 THEN
          ...
          FROM
            viw_tesis ts
          WHERE
            ts.anio LIKE CONCAT('%', pCns, '%');
          IF pRow > 10 THEN
            PREPARE STMT FROM "	SELECT * FROM viw_tesis ts WHERE ts.anio LIKE CONCAT('%',pCns,'%') LIMIT ?,?";
            EXECUTE STMT USING @vLimit, @vEmpiezo;
          ELSE
            SELECT
              *
...
--
-- Definition for procedure SPRCNSOficina
--
DROP PROCEDURE IF EXISTS SPRCNSOficina$$
CREATE PROCEDURE SPRCNSOficina()
BEGIN
...
--
-- Definition for procedure SPRINSMonografia
--
DROP PROCEDURE IF EXISTS SPRINSMonografia$$
CREATE PROCEDURE SPRINSMonografia(IN pCodigoMaterialBibliografico VARCHAR(40), IN pIdEscuela INT, IN pCant TINYINT, IN pSujeto1 BIGINT, IN pSujeto2 BIGINT, IN pTitulo VARCHAR(300), IN pIdAsesor BIGINT, IN pAnio YEAR, IN pResumen TEXT, IN pIntroduccion TEXT, IN pConclusion TEXT, IN pNumeroPagina SMALLINT)
BEGIN
  CALL SPRINSMaterialBibliografico(pCodigoMaterialBibliografico, pTitulo,pIntroduccion ,'' ,pResumen ,pConclusion ,pAnio , '',pNumeroPagina , '', null, '', '', 1, null, 1);
...
IF pCant=1 THEN 
  ...
  INSERT INTO tbl_autor_interno (id_tipo_autor_interno,id_sujeto,id_escuela) 
    VALUES (5,pSujeto1,pIdEscuela);COMMIT;
  ...
  -- INSERTA MATERIAL_BIBLIOGRAFICO CON AUTOR INTERNO
    INSERT INTO tbl_autor_interno_material_bibliografico(id_autor_interno,id_material_bibliografico,id_docente) 
      VALUES (@pIdAutorInterno1,@pIdMaterialBibliografico,pIdAsesor);
  -- TERMINA DE INSERTAR MATERIAL_BIBLIOGRAFICO CON AUTOR INTERNO
  ELSE IF pCant =2 THEN
  ...
  INSERT INTO tbl_autor_interno_material_bibliografico(id_autor_interno,id_material_bibliografico,id_docente) 
    VALUES (@pIdAutorInterno1,@pIdMaterialBibliografico,pIdAsesor);COMMIT;
  INSERT INTO tbl_autor_interno_material_bibliografico(id_autor_interno,id_material_bibliografico,id_docente) 
    VALUES (@pIdAutorInterno2,@pIdMaterialBibliografico,pIdAsesor);COMMIT;
...
IF pIns=1 THEN 
  ...
  -- INSERTA MATERIAL_BIBLIOGRAFICO CON AUTOR INTERNO 1 Y 2
  INSERT INTO tbl_autor_interno_material_bibliografico(id_autor_interno,id_material_bibliografico,id_docente) 
    VALUES (@pIdAutorInterno1,@pIdMaterialBibliografico,pIdDocente);COMMIT;
  INSERT INTO tbl_autor_interno_material_bibliografico(id_autor_interno,id_material_bibliografico,id_docente) 
    VALUES (@pIdAutorInterno2,@pIdMaterialBibliografico,pIdDocente);COMMIT;
  -- TERMINA DE INSERTAR MATERIAL_BIBLIOGRAFICO CON AUTOR INTERNO 1 Y 2
    END If;
  END IF;
END
...
--
DROP VIEW IF EXISTS viw_carrera_profesional CASCADE;
CREATE OR REPLACE 
...
--
DROP VIEW IF EXISTS viw_sujeto CASCADE;
CREATE OR REPLACE 
VIEW viw_sujeto
AS
...
--
DROP VIEW IF EXISTS viw_alumno CASCADE;
CREATE OR REPLACE 
VIEW viw_alumno
AS
...
--
DROP VIEW IF EXISTS viw_material_bibliografico_interno CASCADE;
CREATE OR REPLACE 
...
--
DROP VIEW IF EXISTS viw_tesis CASCADE;
CREATE OR REPLACE 
VIEW viw_tesis
AS
...
--
DROP VIEW IF EXISTS viw_usuario CASCADE;
CREATE OR REPLACE 
VIEW viw_usuario
AS
...
--
DROP VIEW IF EXISTS viw_plan_tesis CASCADE;
CREATE OR REPLACE 
VIEW viw_plan_tesis
AS
...
--
INSERT INTO tbl_oficina VALUES 
  (1, 'Oficina Uno', '', '1'),
  (2, 'Oficina de Biblioteca Central ', NULL, '1'),
  (3, 'Usuario', NULL, '1'),
  (4, 'AdministraciÃ³n', NULL, '1'),
  (5, 'Oficina de AlmacÃ©n', NULL, '0'),
...
INSERT INTO tbl_componente VALUES 
  (77, 'Módulo de Acción', 'Acciones', 26, NULL),
...
INSERT INTO tbl_material_bibliografico VALUES 
  ...
  (45, '0000', 'MONO', 'INTRMON', '', 'RESMON', 'CONCMON', 2011, '', 12, '', NULL, '', '', 1, NULL, '1'),
  (46, '0000', 'MONO', 'INTRMON', '', 'RESMON', 'CONCMON', 2011, '', 12, '', NULL, '', '', 1, NULL, '1');
