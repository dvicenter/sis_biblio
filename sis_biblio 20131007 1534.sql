...
-- Script date 07/10/2013 03:35:15 p.m.
...
BEGIN
  ...
  SET @vEmpiezo = pOffset;
	...
	IF pIdFiltro = 0 THEN
		SELECT COUNT(*) INTO pRow FROM sis_biblio.viw_tesis ts WHERE ts.titulo LIKE CONCAT('%',pCns,'%');
		IF pRow > 10 THEN
	...
	ELSE
		IF pIdFiltro = 1 THEN
			SELECT COUNT(*) INTO pRow FROM viw_tesis ts WHERE ts.alumno LIKE CONCAT('%',pCns,'%');
			IF pRow > 10 THEN
		...
		ELSE
			IF pIdFiltro = 2 THEN
				SELECT COUNT(*) INTO pRow FROM viw_tesis ts WHERE ts.docente LIKE CONCAT('%',pCns,'%');
				IF pRow > 10 THEN
      ...
      EXECUTE STMT USING @vLimit, @vEmpiezo;
			...
			ELSE
				IF pIdFiltro = 3 THEN
					SELECT COUNT(*) INTO pRow FROM viw_tesis ts WHERE ts.anio LIKE CONCAT('%',pCns,'%');
					IF pRow > 10 THEN
...
--
DROP VIEW IF EXISTS viw_material_bibliografico_interno_alumno CASCADE;
CREATE OR REPLACE 
