...
-- Script date 04/10/2013 05:46:01 p.m.
...
CREATE TABLE tbl_alumno_plantesis (
...
CREATE DEFINER = 'root'@'localhost'
PROCEDURE SPRABMAutorInterno(IN pIdAbm TINYINT, IN pIdTipoAutorInterno BIGINT, IN pIdAlumno BIGINT,IN pIdAutorInterno BIGINT)
BEGIN
IF pIdAbm=1 THEN
...
CREATE DEFINER = 'root'@'localhost'
PROCEDURE SPRABMComponenteAccion(IN pIdAbm TINYINT, IN pIdComponenteAccion INT, IN pIdComponente INT, IN pIdAccion TINYINT)
BEGIN
IF pIdAbm=1 THEN
...
CREATE DEFINER = 'root'@'localhost'
PROCEDURE SPRABMRolComponenteAccion(IN pIdAbm TINYINT, IN pIdRolComponenteAccion INT, IN pIdRol BIGINT, IN pIdComponenteAccion INT)
BEGIN
 IF pIdAbm=0 THEN
...
CREATE DEFINER = 'root'@'localhost'
PROCEDURE SPRABMUsuarioRol(IN pIdAbm TINYINT, IN pIdUsuarioRol INT, IN pIdUsuario BIGINT, IN pIdRol INT)
BEGIN
 IF pIdAbm=1 THEN
...
CREATE DEFINER = 'root'@'localhost'
PROCEDURE SPRAPREstadoPlanTesis(IN pIdPlanTesis BIGINT, IN pIdEstado INT)
BEGIN
...
CREATE DEFINER = 'root'@'localhost'
PROCEDURE SPRCNSAutoresXTipoAutor(IN pIdCns TINYINT)
BEGIN
  IF pIdCns=1 THEN 
    SELECT
      viw.id_sujeto,
...
CREATE DEFINER = 'root'@'localhost'
PROCEDURE SPRCNSComponenteAccion(IN pIdCns TINYINT, IN pIdComponente INT, IN pLimit INT)
BEGIN
  IF pIdCns=1 THEN
SELECT
  tblc.id_componente,
...
CREATE DEFINER = 'root'@'localhost'
PROCEDURE SPRCNSInformePractica(IN pTipoCns TINYINT ,IN pIdTipoMaterial TINYINT, IN pIdAlumno BIGINT)
BEGIN
  IF pTipoCns=1 THEN
SELECT
  viw.id_material_bibliografico,
...
CREATE DEFINER = 'root'@'localhost'
PROCEDURE SPRCNSListar_rol_componente_accion(IN pIdCns TINYINT, IN pIdRol INT(11), IN pLimit TINYINT)
BEGIN
IF pIdCns=1 THEN
...
CREATE DEFINER = 'root'@'localhost'
PROCEDURE SPRCNSListar_rol_usuario(IN pIdCns TINYINT,IN pId_usuario BIGINT(20),IN pLimit TINYINT)
BEGIN
 IF pIdCns=1 THEN
SELECT
...
CREATE DEFINER = 'root'@'localhost'
PROCEDURE SPRCNSMaterialBibliograficoXAsesor(IN pIdDocente BIGINT)
BEGIN
SELECT
  viw.id_material_bibliografico,
  viw.titulo,
...
CREATE DEFINER = 'root'@'localhost'
PROCEDURE SPRCNSPlanTesis(IN pIdCns TINYINT, IN pIdPlanTesis INT)
BEGIN
...
--
-- Definition for procedure SPRCNSSujetoAutor
--
DROP PROCEDURE IF EXISTS SPRCNSSujetoAutor$$
CREATE DEFINER = 'root'@'localhost'
PROCEDURE SPRCNSSujetoAutor()
BEGIN
  SELECT    
    viw.sujeto
  FROM
...
CREATE DEFINER = 'root'@'localhost'
PROCEDURE SPRINSSolicitudConstancia(IN pTitulo VARCHAR(300),IN pIntroduccion text,IN pObjetivo text,IN pResumen TEXT, IN pConclusion TEXT,IN pCorreo VARCHAR(50),IN pActivo CHAR(1),IN pIdSujeto1 BIGINT,IN pIdDocente BIGINT,IN pIdSujeto2 BIGINT,IN pIns TINYINT)
BEGIN
  IF pIns=1 THEN
...
CREATE DEFINER = 'root'@'localhost'
PROCEDURE SPRINSTesis(IN pIdAutorInterno BIGINT, IN pIdDocente BIGINT,IN pTitulo VARCHAR(300),IN pIntroduccion TEXT,IN pObjetivo TEXT,IN pResumen TEXT,IN pConclusion TEXT,IN pAnio YEAR,IN pNumeroPagina SMALLINT,IN pCodigoMaterialBibliografico VARCHAR(40))
BEGIN
...
CREATE DEFINER = 'root'@'localhost'
PROCEDURE SPRRPTReporteNegatividad(IN IdPlanTesis BIGINT)
BEGIN
    SELECT
    tbl.titulo,
    (SELECT viwa.alumno 
...
CREATE DEFINER = 'root'@'localhost'
PROCEDURE SPRUPDTesis(IN pIdMaterialBibliografico BIGINT, IN pIntroduccion TEXT, IN pObjetivo TEXT, IN pResumen TEXT, IN pConclusion TEXT)
BEGIN
