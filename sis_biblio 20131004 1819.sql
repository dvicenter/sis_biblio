...
-- Script date 04/10/2013 06:19:14 p.m.
...
BEGIN
 SELECT
  viw.id_docente,
  viw.id_sujeto,
  CONCAT(IFNULL(viw.`apellido_paterno`, ''), ' ', IFNULL(viw.`apellido_materno`, ''),' ' ,IFNULL(viw.`primer_nombre`, '')) AS docente,
  viw.activo
FROM
  viw_docente viw
  WHERE viw.activo='1';
      ...
      SELECT
        ...
        FROM
          ...
          ELSE IF pIdCns=3 THEN -- FUNCIONA PARA LISTAR SOLO A TESISTA
               ...
               ELSE IF pIdCns=4 THEN
                      select viw.id_sujeto 'id_autor_interno',
                        viw.sujeto 'alumno'
                        from viw_sujeto viw
                      WHERE viw.activo=1;
                      END IF;
...
--
-- Definition for procedure SPRCNSFiltro
--
CREATE PROCEDURE SPRCNSFiltro(IN pIdFiltro INT, IN pCns varchar(50), IN pLimit int, IN pOffset INT)
BEGIN

	SELECT @num_row := COUNT(*) FROM viw_tesis;
	
	IF pIdFiltro = 0 THEN
		IF @num_row > 10 THEN
			SELECT * FROM viw_tesis ts WHERE ts.titulo LIKE CONCAT('%',pCns,'%') ;-- LIMIT pLimit OFFSET pOffset;
		ELSE 
			SELECT * FROM viw_tesis ts WHERE ts.titulo LIKE CONCAT('%',pCns,'%');
		END IF;
	ELSE
		IF pIdFiltro = 1 THEN
			IF @num_row > 10 THEN
				SELECT * FROM viw_tesis ts WHERE ts.alumno LIKE CONCAT('%',pCns,'%');-- LIMIT pLimit OFFSET pOffset;
			ELSE 
				SELECT * FROM viw_tesis ts WHERE ts.alumno LIKE CONCAT('%',pCns,'%');
			END IF;
		ELSE
			IF pIdFiltro = 2 THEN
				IF @num_row > 10 THEN
					SELECT * FROM viw_tesis ts WHERE ts.docente LIKE CONCAT('%',pCns,'%'); -- LIMIT pLimit OFFSET pOffset;
				ELSE 
					SELECT * FROM viw_tesis ts WHERE ts.docente LIKE CONCAT('%',pCns,'%');
				END IF;
			ELSE
				IF pIdFiltro = 3 THEN
					IF @num_row > 10 THEN
						SELECT * FROM viw_tesis ts WHERE ts.anio LIKE CONCAT('%',pCns,'%'); -- LIMIT pLimit OFFSET pOffset;
					ELSE 
						SELECT * FROM viw_tesis ts WHERE ts.anio LIKE CONCAT('%',pCns,'%');
					END IF;
				END IF;
			END IF;
		END IF;
...
--
CREATE PROCEDURE SPRINSTesis(IN pSujeto1 BIGINT, IN pIdDocente BIGINT,IN pTitulo VARCHAR(300),IN pIntroduccion TEXT,IN pObjetivo TEXT,IN pResumen TEXT,IN pConclusion TEXT,IN pAnio YEAR,IN pNumeroPagina SMALLINT,IN pCodigoMaterialBibliografico VARCHAR(40),IN pSujeto2 BIGINT,IN pIns CHAR(1),IN pIdEscuela INT)
BEGIN
...
IF pIns=1 THEN 
  -- INSERTAR NUEVO AUTOR INTERNO
  INSERT INTO tbl_autor_interno (id_tipo_autor_interno,id_sujeto,id_escuela) 
    VALUES (5,pSujeto1,pIdEscuela);
  set @pIdAutorInterno1:=(SELECT LAST_INSERT_ID());
  -- TERMINA DE INSERTAR NUEVO AUTOR INTERNO
  -- INSERTA MATERIAL_BIBLIOGRAFICO CON AUTOR INTERNO
    INSERT INTO tbl_autor_interno_material_bibliografico(id_autor_interno,id_material_bibliografico,id_docente) 
      VALUES (@pIdAutorInterno1,@pIdMaterialBibliografico,pIdDocente);
  -- TERMINA DE INSERTAR MATERIAL_BIBLIOGRAFICO CON AUTOR INTERNO
  ELSE IF pIns =2 THEN
  -- INSERTAR NUEVO AUTOR INTERNO 1
  INSERT INTO tbl_autor_interno (id_tipo_autor_interno,id_sujeto,id_escuela) 
    VALUES (5,pSujeto1,pIdEscuela);
  set @pIdAutorInterno1:=(SELECT LAST_INSERT_ID());
  -- TERMINA DE INSERTAR NUEVO AUTOR INTERNO 1
  -- INSERTAR NUEVO AUTOR INTERNO 2
  INSERT INTO tbl_autor_interno (id_tipo_autor_interno,id_sujeto,id_escuela) 
    VALUES (5,pSujeto1,pIdEscuela);
  set @pIdAutorInterno2:=(SELECT LAST_INSERT_ID());
  -- TERMINA DE INSERTAR NUEVO AUTOR INTERNO 2
  -- INSERTA MATERIAL_BIBLIOGRAFICO CON AUTOR INTERNO 1 Y 2
  INSERT INTO tbl_autor_interno_material_bibliografico(id_autor_interno,id_material_bibliografico,id_docente) 
    VALUES (@pIdAutorInterno1,@pIdMaterialBibliografico,pIdDocente);
  INSERT INTO tbl_autor_interno_material_bibliografico(id_autor_interno,id_material_bibliografico,id_docente) 
    VALUES (@pIdAutorInterno2,@pIdMaterialBibliografico,pIdDocente);
  -- TERMINA DE INSERTAR MATERIAL_BIBLIOGRAFICO CON AUTOR INTERNO 1 Y 2
    END If;
  END IF;
END
$$
...
VIEW viw_docente
AS
	select `tbl_d`.`id_docente` AS `id_docente`,`tbl_d`.`codigo_docente` AS `codigo_docente`,`tbl_d`.`codigo_personal` AS `codigo_personal`,`tbl_s`.`id_sujeto` AS `id_sujeto`,`tbl_d`.`id_facultad` AS `id_facultad`,`tbl_s`.`fecha_registro` AS `fecha_registro`,ifnull(`tbl_s`.`apellido_paterno`,'') AS `apellido_paterno`,ifnull(`tbl_s`.`apellido_materno`,'') AS `apellido_materno`,ifnull(`tbl_s`.`primer_nombre`,'') AS `primer_nombre`,ifnull(`tbl_s`.`segundo_nombre`,'') AS `segundo_nombre`,ifnull(`tbl_s`.`tercer_nombre`,'') AS `tercer_nombre`,concat(ifnull(`tbl_s`.`apellido_paterno`,''),' ',ifnull(`tbl_s`.`apellido_materno`,''),' ',ifnull(`tbl_s`.`primer_nombre`,''),' ',ifnull(`tbl_s`.`segundo_nombre`,''),' ',ifnull(`tbl_s`.`tercer_nombre`,'')) AS `docente`,`tbl_s`.`fecha_nacimiento` AS `fecha_nacimiento`,`tbl_s`.`dni` AS `dni`,`tbl_s`.`direccion` AS `direccion`,`tbl_s`.`estado_civil` AS `estado_civil`,`tbl_s`.`sexo` AS `sexo`,`tbl_s`.`telefono_fijo` AS `telefono_fijo`,`tbl_s`.`telefono_movil` AS `telefono_movil`,`tbl_s`.`email_institucional` AS `email_institucional`,`tbl_s`.`email_personal` AS `email_personal`,`tbl_s`.`activo` AS `activo` from (`tbl_docente` `tbl_d` join `tbl_sujeto` `tbl_s` on((`tbl_d`.`id_sujeto` = `tbl_s`.`id_sujeto`)));
...
INSERT INTO tbl_componente VALUES 
  (77, 'MÃ³dulo de AcciÃ³n', ''),
  (84, 'MÃ³dulo de Componente', ''),
  (86, 'MÃ³dulo de Rol', ''),
  (87, 'MÃ³dulo de Usuario', ''),
  (89, 'MÃ³dulo Solicitar Constancia', ''),
  (91, 'MÃ³dulo de AsignaciÃ³n de Roles', ''),
  (93, 'MÃ³dulo de AsignaciÃ³n de Componente AcciÃ³n', ''),
  (94, 'MÃ³dulo de AsignaciÃ³n de AcciÃ³n', ''),
  (96, 'MÃ³dulo Control de Tesis', ''),
  (97, 'MÃ³dulo Cargar Voucher', ''),
  (98, 'MÃ³dulo Solicitudes por Aprobar', '');
...
INSERT INTO tbl_sujeto VALUES 
  ...
  (87, '2013-07-20', 'BRAVO', 'NUÃ‘EZ', 'CARMEN', 'ROSA', '', NULL, '15581004', NULL, NULL, '', NULL, NULL, NULL, NULL, '1'),
  ...
  (118, '2013-07-20', 'CARDENAS', 'SALDAÃ‘A', 'LUIS', 'ALBERTO', '', NULL, '32766171', NULL, NULL, '', NULL, NULL, NULL, NULL, '1'),
  ...
  (123, '2013-07-20', 'CARREÃ‘O', 'CISNEROS', 'EDGARDO', 'OCTAVIO', '', NULL, '03631561', NULL, NULL, 'M', NULL, NULL, NULL, NULL, '1'),
  (124, '2013-07-20', 'CARREÃ‘O', 'MUNDO', 'HUMBERTO', '', '', NULL, '15843945', NULL, NULL, 'M', NULL, NULL, NULL, NULL, '1'),
  (125, '2013-07-20', 'CARREÃ‘O', 'RAMIREZ', 'DANILO', 'HUGO', '', NULL, '15589843', NULL, NULL, '', NULL, NULL, NULL, NULL, '1'),
  ...
  (129, '2013-07-20', 'CASTAÃ‘EDA', 'CARRION', 'YOLANDA', 'MARIANELA', '', NULL, '15598277', NULL, NULL, '', NULL, NULL, NULL, NULL, '1'),
  (130, '2013-07-20', 'CASTAÃ‘EDA', 'CHINGA', 'ANGELICA', '', '', NULL, '15639623', NULL, NULL, '', NULL, NULL, NULL, NULL, '1'),
  ...
  (148, '2013-07-20', 'CONDE', 'CURIÃ‘AUPA', 'REGULO', '', '', NULL, '10177373', NULL, NULL, 'M', NULL, NULL, NULL, NULL, '1'),
  ...
  (158, '2013-07-20', 'CORNELIO', 'VICUÃ‘A', 'MOISES', 'LUIS', '', NULL, '15613382', NULL, NULL, '', NULL, NULL, NULL, NULL, '1'),
  ...
  (162, '2013-07-20', 'CRUZ', 'CASTAÃ‘EDA', 'CARLOS', 'MANUEL', '', NULL, '80593441', NULL, NULL, 'M', NULL, NULL, NULL, NULL, '1'),
  ...
  (202, '2013-07-20', 'DUEÃ‘AS', 'SANCHEZ', 'BENIGNO', 'FELIX', '', NULL, '15584447', NULL, NULL, 'M', NULL, NULL, NULL, NULL, '1'),
  ...
  (218, '2013-07-20', 'ESTUPIÃ‘AN', 'CHUMBES', 'CRISTINA', 'LILLIAN', '', NULL, '15603428', NULL, NULL, 'F', NULL, NULL, NULL, NULL, '1'),
  (219, '2013-07-20', 'ESTUPIÃ‘AN', 'NICHO', 'JESUS', 'EDILBERTO', '', NULL, '15689934', NULL, NULL, '', NULL, NULL, NULL, NULL, '1'),
  ...
  (226, '2013-07-20', 'FARROÃ‘AN', 'SANTISTEBAN', 'JOSE', 'NATIVIDAD', '', NULL, '17520386', NULL, NULL, '', NULL, NULL, NULL, NULL, '1'),
  ...
  (234, '2013-07-20', 'FLORES', 'BRICEÃ‘O', 'RANULFO', '', '', NULL, '17937576', NULL, NULL, 'M', NULL, NULL, NULL, NULL, '1'),
  ...
  (237, '2013-07-20', 'FLORES', 'SALDAÃ‘A', 'OSWALDO', 'FRANCISCO', '', NULL, '15582719', NULL, NULL, '', NULL, NULL, NULL, NULL, '1'),
  ...
  (248, '2013-07-20', 'GARCIA', 'CHAPOÃ‘AN', 'ABRAHAM', 'WILLIAM', '', NULL, '17548877', NULL, NULL, 'M', NULL, NULL, NULL, NULL, '1'),
  ...
  (258, '2013-07-20', 'GONZALES', 'AÃ‘ORGA', 'CARLOS', 'MAXIMO', '', NULL, '15762004', NULL, NULL, '', NULL, NULL, NULL, NULL, '1'),
  ...
  (267, '2013-07-20', 'GOÃ‘Y', 'AMERI', 'CARLOS', 'FRANCISCO', '', NULL, '15726541', NULL, NULL, '', NULL, NULL, NULL, NULL, '1'),
  ...
  (292, '2013-07-20', 'HUAYNA', 'DUEÃ‘AS', 'LUIS', 'ALBERTO', '', NULL, '06057350', NULL, NULL, '', NULL, NULL, NULL, NULL, '1'),
  ...
  (299, '2013-07-20', 'IPANAQUE', 'ROÃ‘A', 'JUAN', 'MANUEL', '', NULL, '32952515', NULL, NULL, 'M', NULL, NULL, NULL, NULL, '1'),
  ...
  (322, '2013-07-20', 'LAVALLE', 'PEÃ‘A', 'GUIDO', 'FABIAN', '', NULL, '15725665', NULL, NULL, '', NULL, NULL, NULL, NULL, '1'),
  ...
  (352, '2013-07-20', 'LLAÃ‘EZ', 'BUSTAMANTE DE ALOR', 'SOLEDAD', 'DIONISIA', '', NULL, '15689024', NULL, NULL, '', NULL, NULL, NULL, NULL, '1'),
  ...
  (359, '2013-07-20', 'MAGUIÃ‘A', 'ARNAO', 'ERNESTO', 'ANDRES', '', NULL, '15617502', NULL, NULL, '', NULL, NULL, NULL, NULL, '1'),
  (360, '2013-07-20', 'MAGUIÃ‘A', 'MAZA', 'RUFINO', 'MAXIMO', '', NULL, '15733560', NULL, NULL, '', NULL, NULL, NULL, NULL, '1'),
  ...
  (364, '2013-07-20', 'MANRIQUE', 'QUIÃ‘ONEZ', 'JAVIER', 'ALBERTO', '', NULL, '15646920', NULL, NULL, 'M', NULL, NULL, NULL, NULL, '1'),
  ...
  (418, '2013-07-20', 'MUÃ‘OZ', 'VILELA', 'ALGEMIRO', 'JULIO', '', NULL, '15736557', NULL, NULL, 'M', NULL, NULL, NULL, NULL, '1'),
  ...
  (421, '2013-07-20', 'NATIVIDAD', 'MUÃ‘OZ', 'ROBERT', 'SANDRO', '', NULL, '15726124', NULL, NULL, '', NULL, NULL, NULL, NULL, '1'),
  ...
  (430, '2013-07-20', 'NOREÃ‘A', 'LUCHO', 'MIRIAM', 'MILAGROS', '', NULL, '15588034', NULL, NULL, '', NULL, NULL, NULL, NULL, '1'),
  ...
  (433, '2013-07-20', 'NUÃ‘EZ', 'TORREBLANCA', 'RANDOLFO', '', '', NULL, '41577711', NULL, NULL, '', NULL, NULL, NULL, NULL, '1'),
  ...
  (441, '2013-07-20', 'ORDOÃ‘EZ', 'VILLAORDUÃ‘A', 'CARMEN', 'GULIANA', '', NULL, '40552763', NULL, NULL, 'F', NULL, NULL, NULL, NULL, '1'),
  (442, '2013-07-20', 'ORMEÃ‘O', 'FLORES', 'FELIX', 'GILBERTO', '', NULL, '21410062', NULL, NULL, 'M', NULL, NULL, NULL, NULL, '1'),
  ...
  (471, '2013-07-20', 'PEÃ‘A', 'AYUDANTE', 'WILLIAM', 'ROGELIO', '', NULL, '15736189', NULL, NULL, '', NULL, NULL, NULL, NULL, '1'),
  (472, '2013-07-20', 'PEÃ‘A', 'CANGA', 'HUGO', 'EDUARDO', '', NULL, '15607951', NULL, NULL, '', NULL, NULL, NULL, NULL, '1'),
  ...
  (476, '2013-07-20', 'PEREZ', 'PIÃ‘AN', 'EDGAR', 'MANUEL', '', NULL, '40499581', NULL, NULL, '', NULL, NULL, NULL, NULL, '1'),
  ...
  (482, '2013-07-20', 'PICHILINGUE', 'NUÃ‘EZ VDA. DE LIENDO', 'FLOR', 'VICTORIA', '', NULL, '15587017', NULL, NULL, '', NULL, NULL, NULL, NULL, '1'),
  ...
  (522, '2013-07-20', 'RODRIGUEZ', 'NUÃ‘EZ', 'JOSE', 'LUIS', '', NULL, '15739966', NULL, NULL, 'M', NULL, NULL, NULL, NULL, '1'),
  ...
  (557, '2013-07-20', 'SALAZAR', 'SANTIBAÃ‘EZ', 'ALEJANDRO', 'MANUEL', '', NULL, '15592485', NULL, NULL, 'M', NULL, NULL, NULL, NULL, '1'),
  ...
  (600, '2013-07-20', 'SUSANIBAR', 'HOCES', 'TEOBALDO', 'NOREÃ‘O', '', NULL, '15688490', NULL, NULL, 'M', NULL, NULL, NULL, NULL, '1'),
  ...
  (627, '2013-07-20', 'VALENZUELA', 'MUÃ‘OZ', 'ALBERTO', '', '', NULL, '15593785', NULL, NULL, '', NULL, NULL, NULL, NULL, '1'),
  ...
  (671, '2013-07-20', 'ZUBIETA', 'NUÃ‘EZ', 'FILOMENO', '', '', NULL, '08297009', NULL, NULL, '', NULL, NULL, NULL, NULL, '1'),
  (672, '2013-07-20', 'ZUÃ‘IGA', 'ROJAS', 'MARCELO', 'GUMERCINDO', '', NULL, '15728921', NULL, NULL, 'M', NULL, NULL, NULL, NULL, '1'),
  (673, '2013-07-20', 'SANTOS', 'ARCE', 'JIMMY', 'ALONSO', '', NULL, '42154689', NULL, NULL, 'M', NULL, NULL, NULL, NULL, '1'),
  (674, '2013-07-20', 'OCROSPOMA', 'DUEÃ‘AS', 'ROBERT', 'WILLIAM', '', NULL, '15728953', NULL, NULL, 'M', NULL, NULL, NULL, NULL, '1'),
  ...
  (701, '2013-07-20', 'VALERIO', 'HARO', 'FÃ‰LIX', 'JULIÃN', '', NULL, '123456', NULL, NULL, 'M', NULL, NULL, NULL, NULL, '1'),
  ...
  (705, '2013-07-20', 'CÃCERES', 'COZ', 'LUZ', 'MARIANELA', '', NULL, '40534860', NULL, NULL, 'F', NULL, NULL, NULL, NULL, '1'),
  (706, '2013-07-20', 'DIAZ', 'GÃ“MEZ', 'JUAN', 'EDILBERTO', '', NULL, '15589865', NULL, NULL, 'M', NULL, NULL, NULL, NULL, '1'),
  ...
  (710, '2013-07-20', 'YARLEQUE', 'WONG', 'JUANA', 'MARÃA', '', NULL, '15996593', NULL, NULL, 'F', NULL, NULL, NULL, NULL, '1'),
  (711, '2013-07-20', 'SÃNCHEZ', 'JARA', 'CESAR', 'CHRISTIAN', '', NULL, '15865076', NULL, NULL, 'M', NULL, NULL, NULL, NULL, '1'),
  ...
  (716, '2013-07-20', 'BASILIO', 'LOVATÃ“N', 'EDWAR', 'DANIEL', '', NULL, '15648773', NULL, NULL, 'M', NULL, NULL, NULL, NULL, '1'),
  ...
  (721, '2013-07-20', 'LOZA', 'ROSADIO', 'FÃ‰LIX', 'MOISES', '', NULL, '15594052', NULL, NULL, 'M', NULL, NULL, NULL, NULL, '1'),
  (722, '2013-07-20', 'TORRES', 'CALDERÃ“N', 'ALEX', 'FIDEL', '', NULL, '1254678', NULL, NULL, 'M', NULL, NULL, NULL, NULL, '1'),
  ...
  (730, '2013-07-20', 'VARGAS', 'MAGUIÃ‘A', 'TEOFILO', 'TOMAS', '', NULL, '15689353', NULL, NULL, 'M', NULL, NULL, NULL, NULL, '1'),
  ...
  (734, '2013-07-20', 'CASTAÃ‘EDA', 'SAMANAMU', 'MIGURL', 'A.', '', NULL, '15726159', NULL, NULL, 'F', NULL, NULL, NULL, NULL, '1'),
  ...
  (738, '2013-07-20', 'TERRONES', 'GÃLVEZ', 'EDWARD', '', '', NULL, '00000000', NULL, NULL, 'M', NULL, NULL, NULL, NULL, '1'),
  (739, '2013-07-20', 'MANRIQUE', 'JIMENEZ', 'CARLOS', 'ANDRÃ‰S', '', NULL, '00000000', NULL, NULL, 'M', NULL, NULL, NULL, NULL, '1'),
  (740, '2013-07-20', 'ARREDONDO', 'GUERRERO', 'GUSTAVO', 'MATIAS', '', NULL, '00000000', NULL, NULL, 'M', NULL, NULL, NULL, NULL, '1'),
  (741, '2013-07-20', 'GARCIA', 'SANTIVAÃ‘EZ', 'MOISES ', 'HUMBERTO', '', NULL, '00000000', NULL, NULL, 'M', NULL, NULL, NULL, NULL, '1'),
  ...
  (744, '2013-07-20', 'PACHECO', 'PEÃ‘A', 'PEDRO', '', '', NULL, '00000000', NULL, NULL, 'M', NULL, NULL, NULL, NULL, '1'),
  (745, '2013-07-20', 'MEDINA', 'MEDINA', 'JOSÃ‰ ', 'JULIÃN', '', NULL, '00000000', NULL, NULL, 'M', NULL, NULL, NULL, NULL, '1'),
  ...
  (748, '2013-07-20', 'PICHILINGUE', 'MUGURUZA', 'HUGO', 'CÃ‰SAR', '', NULL, '00000000', NULL, NULL, 'M', NULL, NULL, NULL, NULL, '1'),
  (749, '2013-07-20', 'ZUMARAN ', 'IRRIBAREN', 'JOSÃ‰', 'LUIS', '', NULL, '00000000', NULL, NULL, 'M', NULL, NULL, NULL, NULL, '1'),
  (750, '2013-07-20', 'NARBASTA', 'VELASQUEZ', 'JOSÃ‰', 'ELEODORO', '', NULL, '00000000', NULL, NULL, 'M', NULL, NULL, NULL, NULL, '1'),
  ...
  (780, '2013-07-20', 'CASTRO', 'BARTOLOMÃ‰', 'HECTOR', 'JORGE', '', NULL, '12345678', NULL, NULL, 'M', NULL, NULL, NULL, NULL, '1'),
  (836, '2013-07-20', 'MORALES', 'GÃ“MEZ', 'OTTO', 'MAVILIO', '', NULL, '12345678', NULL, NULL, 'M', NULL, NULL, NULL, NULL, '1'),
  ...
  (839, '2013-07-20', 'ZUMARÃN', 'IRRIBARREN', 'JOSÃ‰', 'LUIS', '', NULL, '32885940', NULL, NULL, 'M', NULL, NULL, NULL, NULL, '1'),
  (840, '2013-07-20', 'LOAYZA ', 'SOLIER', 'GILBERTO', 'TITO', '', NULL, '12345678', NULL, NULL, 'M', NULL, NULL, NULL, NULL, '1'),
  (842, '2013-07-20', 'MANDAMIENTO', 'GRADOS', 'ROMEL', 'IVÃN', '', NULL, '12345678', NULL, NULL, 'M', NULL, NULL, NULL, NULL, '1'),
  (843, '2013-07-20', 'CERNA', 'LÃ“PEZ', 'WALTER', 'CÃ‰SAR', '', NULL, '12345678', NULL, NULL, 'M', NULL, NULL, NULL, NULL, '1'),
  ...
  (851, '2013-07-20', 'HUAMÃN', 'FLORES', 'MIGUEL', 'FRANCISCO', '', NULL, '12345678', NULL, NULL, 'M', NULL, NULL, NULL, NULL, '1'),
  (852, '2013-07-20', 'GALDOS', 'FELIPE', 'JOSÃ‰', 'ANTONIO', '', NULL, '12345678', NULL, NULL, 'M', NULL, NULL, NULL, NULL, '1'),
  (853, '2013-07-20', 'REYES', 'NAVARRO', 'FELIPE', 'AMÃ‰RICO', '', NULL, '12345678', NULL, NULL, 'M', NULL, NULL, NULL, NULL, '1'),
...
INSERT INTO tbl_tematica VALUES 
  (1, 'Ciencias', NULL, '1'),
  (2, 'IngenierÃ­a', NULL, '1'),
  (3, 'Medicina', NULL, '1'),
  (4, 'Contabilidad', NULL, '1'),
  (5, 'MatemÃ¡tica', NULL, '1'),
  (6, 'FÃ­sica', NULL, '1');
...
INSERT INTO tbl_tipo_material_bibliografico VALUES 
  (1, 'Tesis', NULL, '1'),
  (2, 'MonografÃ­a', NULL, '1'),
  (3, 'Informe de PrÃ¡ctica', NULL, '1');
...
INSERT INTO tbl_material_bibliografico VALUES 
  ...
  (10, '0', 'TESIS 400', 'INTRODUCCIÃ“N CARÃ‘O', 'INTRODUCCIÃ“N CARÃ‘O', 'INTRODUCCIÃ“N CARÃ‘O', 'INTRODUCCIÃ“N CARÃ‘O', 2010, '', NULL, '', NULL, '', '', 1, NULL, '1'),
  (11, '0', 'TESIS PRIMERO', 'INTROOO', 'OBJT', 'RESUMEN', 'CONCLU', 2000, '', NULL, '', NULL, '', '', 1, NULL, '1'),
  (12, '0', 'TESIS 4', 'Ã±alskd', 'Ã±aldsk', 'Ã±alsdk', 'Ã±lzdkads', 2013, '', NULL, '', NULL, '', '', 1, NULL, '1'),
  (13, '0', 'TESIS 5', 'Ã±alkdÃ±aklsd', 'Ã±lasdkÃ±laskd', 'Ã±laskdÃ±lasd', 'Ã±alsdkÃ±alsdads', 2012, '', NULL, '', NULL, '', '', 1, NULL, '1'),
  (14, '0', 'TESIS DE', 'alskdlakÃ±sjd', 'Ã±alsdkkÃ±alsdk', 'Ã±alksdkÃ±lasd', 'Ã±aldkÃ±laskÃ±lds', 2012, '', NULL, '', NULL, '', '', 1, NULL, '1'),
  ...
  (18, '0', 'CESAR', 'Ã±lasdkÃ±asd', 'Ã±lasdkÃ±lasd', 'Ã±alkdsÃ±', 'Ã±alskdasld', 2012, '', NULL, '', NULL, '', '', 1, NULL, '1'),
  (19, '0', 'TESIS DE REINGENERÍA', 'OK', 'OK', 'OK', 'OK', 2011, '', NULL, '', NULL, '', '', 1, NULL, '1'),
  (20, '0', 'TESIS DE REINGENERÍA', 'OK', 'OK', 'OK', 'OK', 2011, '', NULL, '', NULL, '', '', 1, NULL, '1'),
  (21, '0', 'TESIS DE REINGENERÍA', 'OK', 'OK', 'OK', 'OK', 2011, '', NULL, '', NULL, '', '', 1, NULL, '1'),
  (22, '0', 'TESIS DE REINGENERÍA', 'OK', 'OK', 'OK', 'OK', 2011, '', NULL, '', NULL, '', '', 1, NULL, '1'),
  (23, '0', 'TESIS DE NO SE DE QUE PERO DE ALGO SERVIRA', 'AS', 'AS', 'AS', 'AS', 2011, '', NULL, '', NULL, '', '', 1, NULL, '1'),
  (24, '0', 'TESIS DE NO SE DE QUE PERO DE ALGO SERVIRA', 'AS', 'AS', 'AS', 'AS', 2011, '', NULL, '', NULL, '', '', 1, NULL, '1'),
  (25, '0', 'TESIS DE DOS PERSONAS', 'NO', 'MO', 'NNE', 'DA', 2014, '', NULL, '', NULL, '', '', 1, NULL, '1'),
  (26, '0', 'TESIS DE DOS PERSONAS', 'NO', 'MO', 'NNE', 'DA', 2014, '', NULL, '', NULL, '', '', 1, NULL, '1'),
  (27, '0', 'TESIS DE DOS PERSONAS', 'NO', 'MO', 'NNE', 'DA', 2014, '', NULL, '', NULL, '', '', 1, NULL, '1'),
  (28, '0', 'CHOLO', 'LSKDFJ', '{SFL', 'ÑASLKF', 'ÑALKSD', 2014, '', NULL, '', NULL, '', '', 1, NULL, '1'),
  (29, '0', 'CHOLO', 'LSKDFJ', '{SFL', 'ÑASLKF', 'ÑALKSD', 2014, '', NULL, '', NULL, '', '', 1, NULL, '1'),
  (30, '0', 'CHOLO', 'LSKDFJ', '{SFL', 'ÑASLKF', 'ÑALKSD', 2014, '', NULL, '', NULL, '', '', 1, NULL, '1'),
  (31, '0', 'VVVVVÑ{DSKF', '.ASKDFJ', '{ÑDFSKL', '{LSKDFÑSLDF', 'ÑSLDKFÑLDSF', 2013, '', NULL, '', NULL, '', '', 1, NULL, '1'),
  (32, '0', 'VVVVVÑ{DSKF', '.ASKDFJ', '{ÑDFSKL', '{LSKDFÑSLDF', 'ÑSLDKFÑLDSF', 2013, '', NULL, '', NULL, '', '', 1, NULL, '1'),
  (33, '0', 'VVVVVÑ{DSKF', '.ASKDFJ', '{ÑDFSKL', '{LSKDFÑSLDF', 'ÑSLDKFÑLDSF', 2013, '', NULL, '', NULL, '', '', 1, NULL, '1'),
  (34, '0', 'VVVVVÑ{DSKF', '.ASKDFJ', '{ÑDFSKL', '{LSKDFÑSLDF', 'ÑSLDKFÑLDSF', 2013, '', NULL, '', NULL, '', '', 1, NULL, '1'),
  (35, '0', 'VVVVVÑ{DSKF', '.ASKDFJ', '{ÑDFSKL', '{LSKDFÑSLDF', 'ÑSLDKFÑLDSF', 2013, '', NULL, '', NULL, '', '', 1, NULL, '1'),
  (36, '0', 'VVVVVÑ{DSKF', '.ASKDFJ', '{ÑDFSKL', '{LSKDFÑSLDF', 'ÑSLDKFÑLDSF', 2013, '', NULL, '', NULL, '', '', 1, NULL, '1'),
  (37, '0', 'VVVVVÑ{DSKF', '.ASKDFJ', '{ÑDFSKL', '{LSKDFÑSLDF', 'ÑSLDKFÑLDSF', 2013, '', NULL, '', NULL, '', '', 1, NULL, '1'),
  (38, '0', 'VVVVVÑ{DSKF', '.ASKDFJ', '{ÑDFSKL', '{LSKDFÑSLDF', 'ÑSLDKFÑLDSF', 2013, '', NULL, '', NULL, '', '', 1, NULL, '1'),
  (39, '0', 'VVVVVÑ{DSKF', '.ASKDFJ', '{ÑDFSKL', '{LSKDFÑSLDF', 'ÑSLDKFÑLDSF', 2013, '', NULL, '', NULL, '', '', 1, NULL, '1'),
  (40, '0', 'VVVVVÑ{DSKF', '.ASKDFJ', '{ÑDFSKL', '{LSKDFÑSLDF', 'ÑSLDKFÑLDSF', 2013, '', NULL, '', NULL, '', '', 1, NULL, '1'),
  (41, '0', 'VVVVVÑ{DSKF', '.ASKDFJ', '{ÑDFSKL', '{LSKDFÑSLDF', 'ÑSLDKFÑLDSF', 2013, '', NULL, '', NULL, '', '', 1, NULL, '1'),
  (42, '0', 'VVVVVÑ{DSKF', '.ASKDFJ', '{ÑDFSKL', '{LSKDFÑSLDF', 'ÑSLDKFÑLDSF', 2013, '', NULL, '', NULL, '', '', 1, NULL, '1'),
  (43, '0', 'VVVVVÑ{DSKF', '.ASKDFJ', '{ÑDFSKL', '{LSKDFÑSLDF', 'ÑSLDKFÑLDSF', 2013, '', NULL, '', NULL, '', '', 1, NULL, '1'),
  (44, '0', 'VVVVVÑ{DSKF', 'INTRODUCCION 200kkk', 'OBJETIVO 200', 'RESUMEN200', 'CONCLUSION200', 2013, '', NULL, '', NULL, '', '', 1, NULL, '1');
...
INSERT INTO tbl_plan_tesis VALUES 
  ...
  (33, 'TÃ‰SIS NUMERO 100', 'INTRODUCCIÃ“N 100', 'OBJETIVO 100', 'RESUMEN 100', 'CONCLUSIÃ“N 100', '1', NULL, '0332101040@unjfsc.edu.pe', 3, 1),
  (34, 'TÃ‰SIS200', 'INTRO200', 'OBJETIVO200', 'RESUMEN200', 'CONCLUSION200', '1', NULL, 'cesar@hotmail.com', 4, 1),
  ...
  (39, 'CESAR', 'lksjdfljkdsf', 'Ã±sldkfkskÃ±ldf', 'Ã±skkÃ±sdfkÃ±ldfs', 'Ã±sldfkÃ±lsdkÃ±lfd', '1', NULL, 'cesar@hotmail.com', 14, 3),
  ...
  (43, 'gyfgy', 'hgaidsughasui', 'Ã±laskdÃ±aslkd', 'Ã±alkdÃ±lasdk', 'Ã±alskdÃ±asldk', '1', NULL, 'c@hotmail.com', 1, 2),
  (44, 'CESAR', 'cesar', 'luis', 'nicho', 'diaz', '1', NULL, 'cesar@hotmail.com', 3, 3),
  (45, 'PLAN DE TESIS', 'INTRODUCCIÃ“N PLAN DE TESIS', 'OBJETIVO PLAN DE TESIS', 'RESUMEN PLAN DE TESIS', 'CONCLUCIÃ“N PLAN DE TESIS', '1', NULL, 'cesar2010_13@hotmail.com', 52, 1),
  (46, 'PLAN DE TESIS1', 'INTRO PLAN DE TRSIS1', 'RESUMEN PLAN DE TESIS 1', 'RESUMEN PLAN DE TESIS 1', 'CONCLUSIÃ“N PLAN DE TESIS 1', '1', NULL, 'cesar2010_13@hotmail.com', 52, 2),
  (47, 'PLAN DE TESIS2', 'INTRO', 'OBJ', 'RESUM', 'CONCLUSI', '1', NULL, 'cesar@hotmail.com', 52, 3),
  (48, 'TITU', 'kajsdklasjd', 'aÃ±lskdlaÃ±sdkas', 'laÃ±kdsÃ±laskdlas', 'aÃ±ldskÃ±aslkdlas', '1', NULL, 'cesar@hotmail.com', 3, 3),
  (49, 'TITU', 'LADKJSLASKDJ', 'LÃ‘AKDÃ‘LADSK', 'Ã‘LASKDÃ‘ALSD', 'Ã‘ALSDKÃ‘ASLDK', '1', NULL, 'cesar@hotmail.com', 1, 2),
